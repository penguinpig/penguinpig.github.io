---
author: ["PenguinPig"]
title: "Claude Courses101"
date: "2026-06-11T07:50:00.000Z"
description: "Claude免費課程"
summary: "Claude免費課程"
tags: ["AI"]
categories: ["note"]
series: []
ShowToc: true
TocOpen: true
draft: true
notion_id: "37cb8bba-f3ba-801b-a969-c9683ce95005"
---
## 📥 收集區（先丟進來就好）

### 內容／片段

- Research Mode
  - **Be specific about your goals.**

  - **Specify the sections or structure you want**

  - **Include relevant constraints.**
    - Budget

- 對話疊帶方式
  1. 使用第一個Prompt定錨，檢視輸出哪個對哪個不需要

  1. 根據第一個給出回饋

  1. 了解何時該使用新的上下文

- Prompt的原則
  1. Setting the stage
    1. Role、Objective、Context

  1. Define an task
    1. what action you want Claude to take

  1. Specifying rule
    1. style, tone, and examples

  1. **Ask Claude to help refine your prompt**

- 單字
  - capabilities (n. 能力、水準)

  - variety (n. 多樣化)

  - discriminatory (a. 歧視的)
    - Business leaders must take a stand against discriminatory practices.

  - unethical (a. 不道德的)

  - Constitutional (a. 憲法、符合憲法的)
    - Such a policy would not be constitutional.

  - transparently

  - collaborative (a. 協作的)

  - **steerable **

  - augment (v. 增強)
    - He would have to find work to augment his income.

  - excels (v. 擅長)
    - to be extremely good at something

  - ingest (v. 攝取、嚥下)
    - The chemicals can be poisonous if ingested.

  - cognitive (a. 感知的)
    - Some of her cognitive functions have been impaired.

  - assumption (n. 假定)
    - People tend to make assumptions about you when you have a disability.

  - concisely (a. 簡潔地)
    - The information was presented clearly and concisely.

  - competencies(n. 技術能力)

  - Delegation

  - **discernment (n. 眼力、眼光)**
    - It's clear that you are a person of discernment.

  - Diligence (n. 勤奮、認真)
    - She hoped that her diligence would be noticed at work.

  - Fluency (n. 流利、熟練)
    - One of the requirements of the job is fluency in two or more African languages.

  - **occasionally (ad. 偶爾)**
    - I see him occasionally in town.

  - recurring (a. 反覆的)
    - For much of his life he suffered from recurring bouts of depression.

  - intuition (n. 直覺)
    - I can't explain how I knew - I just had an intuition that you'd been involved in an accident

  - underneath (a. 在..下面0
    - They found a bomb underneath the car.

  - **Dictation (n. 口述)**
    - I'll ask my assistant to take dictation

  - jotting (n. 草草記下)
    - Could you jot your address and phone number in my address book?

  - **tackle (v. 對付、處理)**
    - There are many ways of tackling this problem.

  - stamina (n. 耐力)
    - the physical and/or mental strength to do something that might be difficult and will take a long time

  - briefs (a. 簡短的)
    - It'll only be a brief visit because we really don't have much time.

  - coordinates (v. 協調)
    - We need someone to coordinate the whole campaign.

  - **Dispatch (v. 派遣)**
    - o send something, especially goods or a message, somewhere for a particular purpose

  - buried (v. 埋、安葬)
    - to put a dead body into the ground

  - **criterion (n. 標準)**
    - a standard by which you judge, decide about, or deal with something

  - seamlessly (a. 順利地)
    - without any sudden changes, interruptions, or problems

  - **Revocable (a. 撤銷的)**
    - If an agreement, a law, etc. is revocable, someone can say officially that it is no longer in effect

  - synthesizing (v. 合成)
    - to produce a substance by a chemical reaction in plants or animals

  - unified (a. 統一的)
    - brought together, combined, or united

  - **Citations (n. 引用)**
    - a word or piece of writing taken from a written work

  - briefings (n. 簡報)
    - information that is given to someone just before they do something, or a meeting where this happens

  - pursuing (v. 追捕)
    - information that is given to someone just before they do something, or a meeting where this happensto follow someone or something, usually to try to catch him, her, or i

  - inspiration( n. 啟發、靈感)
    - someone or something that gives you ideas for doing something

  - accelerate (v. 加速)
    - someone or something that gives you ideas for doing somethingWhen a vehicle or its driver accelerates, the speed of the vehicle increases.

  - leverage (n. 槓桿作用)
    - the action or advantage of using a lever

  - tedious (a. 無趣的)
    - broing

  - tighten (v. 緊固)
    - the action or advantage of using a leverto become tighter or to make something become tighter, firmer, or less easy to move

  - dense (a. 濃密的)
    - having parts that are close together so that it is difficult to go or see through

  - encompasses (v. 包含)
    - to include different types of things

  - **Elapsed (v. 流逝)**
    - If time elapses, it goes past.

-

### 來源

- 連結：

- 截圖／檔案：

### 關鍵字（你之後會怎麼找回來？）

-

---

## 🧹 整理區（有空才做）

### 這段片段可以變成什麼？

- 可重做的步驟（操作指南）

- 可重用的 code（程式片段）

- 一句話理解（概念）

### 下一次要做的最小整理

- [ ] 補一句摘要

- [ ] 加 1–3 個 Tag

- [ ] 設定重要度

- [ ] 決定要不要勾精選
